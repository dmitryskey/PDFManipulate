...
# sudo docker run --name smarti9 --net dockerwp -v smarti9:/var/www/html -v ~/uploads.ini:/usr/local/etc/php/conf.d/uploads.ini -e WORDPRESS_DB_HOST=mariadb:3306 -e WORDPRESS_DB_NAME=smarti9 -e WORDPRESS_DB_USER=wordpress -e WORDPRESS_DB_PASSWORD=55187d7fc100164cdb944ba0f5bf3aae397dbf44ea885296 -e VIRTUAL_HOST=smarti9.southcentralus.cloudapp.azure.com -e LETSENCRYPT_HOST=smarti9.southcentralus.cloudapp.azure.com -e LETSENCRYPT_EMAIL=admin@smarti9.southcentralus.cloudapp.azure.com -d --restart always wordpress

sudo docker restart smarti9

echo "Install Smart-Forms-On-Demand plugin"
sudo docker cp ../../build/smartformsondemand.tar.gz smarti9:/tmp/
sudo docker exec -it smarti9 mkdir /tmp/build && mkdir/tmp/build/smartformsondemand
sudo docker exec -it smarti9 tar -xzf /tmp/smartformsondemand.tar.gz -C /tmp/build/smartformsondemand
sudo docker exec -it smarti9 cp -r /tmp/build/smartformsondemand $pluginspath
sudo docker exec -it smarti9 rm -r /tmp/build/
sudo docker exec -it smarti9 rm /tmp/smartformsondemand.tar.gz
